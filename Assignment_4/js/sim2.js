...
let max_cell_volume = 400
let cell_volume_step = 50

let min_cell_density = 0
...
fs.appendFile(
  filename,
  'cell_volume, cell_density, barrier_density, number_of_cells, mean_speed, mean_xdir, mean_ydir \n',
  function (err) {
    if (err) throw err
    console.log('Saved!')
  },
    ...
    for (
      ...
      if (!(cell_density == barrier_density)) {
        ...
        function makeCircularBarriers() {
          ...
          let step_size_x = Math.floor(
            this.C.extents[0] / (barrier_density + 1),
          )
          let step_size_y = Math.floor(
            this.C.extents[1] / (barrier_density + 1),
          )
